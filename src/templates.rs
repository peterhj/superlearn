...
pub struct CategoricalNLLLoss<Input, Label, Prob, Loss> {
  ...
  pub input:        Rc<AVar<AData<Input>>>,
  pub label:        Rc<AVar<AData<Label>>>,
  pub prob:         Rc<AVar<AData<Prob>>>,
  pub loss:         Rc<AVar<AData<Loss>>>,
...
pub struct ClassifierIo<In> {
  pub x:        Rc<SrcOp<In>>,
  pub y_label:  Rc<SrcOp<Batch<u32>>>,
  pub y:        Rc<AVar<AData<BatchArray1d<f32>>>>,
}
...
pub struct RegressorIo<In, Out> {
  pub x:        Rc<SrcOp<In>>,
  pub y_target: Rc<SrcOp<Out>>,
  pub y:        Rc<AVar<AData<Out>>>,
}
